...
function [Gx, Gy] = my_imgradientxy(im)
    ...
    %creates 2 arrays of horizontal and vertial gradients for the
    %multiplication
    ...
    %empty array for growing local graident X and Y
    Gx = [];
    Gy = [];
    ...
    for i=1:1:size(im,1)
        for j=1:1:size(im,2)
           ...
           if(j+2 <= size(im,1) && i+2 <= size(im,1))
                ...
                %all elements are multipled together to create new image
                %convolution patch
                PreX = GxFil.*A;
                PreY = GyFil.*A;
